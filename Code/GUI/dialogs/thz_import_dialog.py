...
class THzImportDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.line_min_intensity = QLineEdit(self)
        self.line_min_intensity.setAlignment(Qt.AlignRight)
        self.line_min_intensity.setText("0.0")
        self.line_min_intensity.setToolTip(
            "Minimum FFT amplitude in arbitrary units to keep in the pseudo-3D "
            "mesh and point cloud. Pixels below this value are flattened to zero "
            "height. Applies when pseudo depth is above 0."
        )
        self.line_min_intensity.setStyleSheet("background-color: #f0f0f0;")
        intensity_validator = QDoubleValidator(0.0, 1.0e12, 6, self)
        intensity_validator.setNotation(QDoubleValidator.StandardNotation)
        self.line_min_intensity.setValidator(intensity_validator)
        form.addRow("Minimum Mesh Intensity (a.u.)", self.line_min_intensity)
        ...
        self.label_note = QLabel(
            ...
            "a registration-friendly 3D height map. Use the minimum mesh intensity to suppress "
            "weak background in the pseudo-3D geometry."
        )
    ...
    def _validate_and_accept(self):
        ...
        intensity_text = self.line_min_intensity.text().strip()
        ...
        if not intensity_text:
            QMessageBox.warning(self, "Missing Intensity", "Please enter a minimum mesh intensity in a.u.")
            return

        try:
            min_intensity_au = float(intensity_text)
        except ValueError:
            QMessageBox.warning(self, "Invalid Intensity", "Minimum mesh intensity must be a valid decimal number.")
            return

        if min_intensity_au < 0.0:
            QMessageBox.warning(self, "Invalid Intensity", "Minimum mesh intensity cannot be negative.")
            return
    ...
    def get_import_params(self) -> THzImportParams:
        ...
        return THzImportParams(
            ...
            min_intensity_au=float(self.line_min_intensity.text().strip() or "0.0"),
        )
