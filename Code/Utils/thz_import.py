...
def build_thz_fft_npy_volume(
    ...
    min_intensity_au = max(float(min_intensity_au), 0.0)
    intensity_min_au = float(np.min(image_yx)) if image_yx.size else 0.0
    intensity_max_au = float(np.max(image_yx)) if image_yx.size else 0.0
    if pseudo_depth_mm > 0.0:
        volume_zyx, pseudo_meta = _build_height_map_volume(
            image_yx=image_yx,
            pixel_size_mm=pixel_size_mm,
            pseudo_depth_mm=pseudo_depth_mm,
            min_intensity_au=min_intensity_au,
        )
        if pseudo_meta["kept_pixels"] <= 0:
            raise ValueError(
                "No THz pixels remained above the minimum mesh intensity of "
                f"{min_intensity_au:.6g} a.u. Lower the threshold or choose a different FFT "
                f"frequency. Current image range is {intensity_min_au:.6g} to "
                f"{intensity_max_au:.6g} a.u."
            )
    ...
    return {
        ...
        "min_intensity_au": min_intensity_au,
        "intensity_min_au": intensity_min_au,
        "intensity_max_au": intensity_max_au,
        "mesh_kept_pixels": pseudo_meta["kept_pixels"],
        "mesh_kept_fraction": pseudo_meta["kept_fraction"],
    }
...
def _build_height_map_volume(
    ...
    if image.size == 0:
        return np.zeros((nz, 1, 1), dtype=np.float32), {
            "z_slices": nz,
            "depth_mm": actual_depth_mm,
            "kept_pixels": 0,
            "kept_fraction": 0.0,
        }

    kept_mask = np.isfinite(image) & (image >= float(min_intensity_au))
    kept_pixels = int(np.count_nonzero(kept_mask))
    kept_fraction = float(kept_pixels) / float(image.size)
    height_voxels = np.zeros(image.shape, dtype=np.int32)
    if kept_pixels > 0:
        kept_values = image[kept_mask]
        lo, hi = np.percentile(kept_values, (1.0, 99.5))
        lower = max(float(min_intensity_au), float(lo) if np.isfinite(lo) else float(min_intensity_au))
        if not np.isfinite(hi) or hi <= lower:
            height_voxels[kept_mask] = 1
        else:
            normalized = np.clip(
                (kept_values - lower) / float(hi - lower),
                0.0,
                1.0,
            ).astype(np.float32, copy=False)
            height_voxels[kept_mask] = np.clip(
